...
// import EditManageMaster from "./EditTableMaster/EditManageMaster";
...
import EditManageMaster from "./EditManageMaster";
import UploadManageMaster from "./UploadManageMaster";
// import UploadManageMaster from "./Upload/UploadManageMaster";
...
function ManageMaster(props) {
  ...
  const columns = [
    ...
    },
    {
       ...
       renderCell: (params) => {
         return <EditManageMaster data={params.row} index={params.row.id} />;
   },
     },
  ];
  ...
  const rows = props.managemaster?.isLoading
    ? []
    : props.managemaster?.properties?.length > 0
    ? props.managemaster?.properties.filter((item) => {
  ...
  useEffect(() => {
    props.onManageMasterGetData(data);
  }, []);
  ...
  console.log("managemaster", props.managemaster);
  // const viewStatus =
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    ...
    props.onPostManageMasterData(data, user, toggle, setSubmitting);
    return;
  };
    ...
    <Card className="p-3 w-100">
      <CardHeader className="bg-gradient-pink p-2 text-white">
        <div className="d-flex align-items-center justify-content-between">
          <strong className="pl-2">ManageMaster Master</strong>
          <div>
            {createStatus && (
              <Button
        ...
        <Modal className="modal-md" isOpen={modal} toggle={toggle}>
          <ModalHeader toggle={toggle} className="d-flex align-items-center">
            Add New ManageMaster Master{" "}
          </ModalHeader>

          {props.managemaster?.isPostLoading && <LinerLoader />}
          <ModalBody className="">
            <Formik
              initialValues={{
                sitename: "",
      ...
      <CardBody style={{ height: 520, width: "100%" }}>
        <DataGrid
          rows={rows}
          columns={columns}
          loading={props.managemaster?.isLoading ? true : false}
          pageSize={pageSize}
...
const mapStateToProps = (state) => {
  return {
    managemaster: state.entities.managemaster.managemaster,
    login: state.login,
  };
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    onDeleteManageMaster: (id, data) =>
      dispatch(actions.deleteManageMaster(id, data)),
