...
function EditMenu(props) {
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    ...
    props.updateMenuData(data, user, toggle, setSubmitting);
    return;
  };
  ...
  return (
    <div>
      ...
      {deleteStatus && (
        <DeleteButton
          id={props.data?.id}
          deleteFunction={() => props.onDeleteMenu(props.data?.id, data)}
        />
      )}
      ...
      <Modal className="modal-md" isOpen={modal} toggle={toggle}>
        ...
        {props.menu.isUpdateLoading && <LinerLoader />}
        <ModalBody className="">
          <Formik
            initialValues={{
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    onDeleteMenu: (id, data) => dispatch(actions.deleteMenu(id, data)),
